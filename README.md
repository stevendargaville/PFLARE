...
2) Other hyperbolic problems such as time independent streaming operators from Boltzmann transport applications.
...
This library depends on MPI, BLAS, LAPACK (>= 3.4) and PETSc (>= 3.14) configured with a graph partitioner (e.g., ParMETIS). Please compile PETSc directly from the source code, as PFLARE requires access to some of the PETSc types only available in the source.
...
3) ``-pc_air_poly_sparsity_order=1`` (default)
...
The CF splittings in PFLARE are used within PCAIR to form the multigrid hierarchy. There are several different CF splittings available with ``-pc_air_cf_splitting_type``:
   ...
   | agg  |  CF_AGG  | Aggregation method with root-nodes as C points - Serial only |
